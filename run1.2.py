...
for run in range(config.run_times):
    ...
    optimizer_init = optim.SGD(model.parameters(), lr=config.lr_init, momentum=0.8)
    scheduler_init = torch.optim.lr_scheduler.StepLR(optimizer_init, step_size=config.patience*2, gamma=0.1)
    # scheduler_init = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_init, T_max=5)
    # ------------------------------------ step 4/5 : training --------------------------------------------------
    # training basic task
    ...
    model, _, _, avg_valid_losses, _, _, hessian_val = train_es(config, basic_task_data, basic_task_test_data, model,
                                                                criterion, optimizer_init,
                                                                scheduler_init, config.max_epoch, config.device,
    ...
    # save task 1
    trained_model_path = save_checkpoint(model, task_id[config.basic_task_index])
    ...
    for j, (train_data, test_data) in enumerate(zip(train_stream, test_stream)):
        ...
        criterion = nn.CrossEntropyLoss()
        optimizer = optim.SGD(trained_model.parameters(), lr=config.lr_init*0.1, momentum=0.9, dampening=0.1)
        ...
        # save model
        save_checkpoint(trained_model, task_id[j])
