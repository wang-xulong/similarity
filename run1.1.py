...
for run in range(config.run_times):
    ...
    model, _, _, avg_valid_losses, _, _ = trainES(basic_task_data, basic_task_test_data, model, criterion, optimizer,
                                                  scheduler, config.max_epoch, device, patience=config.patience,
                                                  task_id=config.basic_task, func_sim=False)

    # setting stage 1 matrix
    acc_array1 = np.zeros((4, 2))
    # testing basic task
    _, acc_array1[:, 0] = test(test_stream[config.basic_task], model, criterion, device, task_id=config.basic_task)
    ...
    for i, probe_data in enumerate(test_stream):
        with torch.no_grad():
            _, acc_array1[i, 1] = test(probe_data, model, criterion, device, task_id=task_id[i])
    # save task 1
    PATH = "./"
    ...
    for j, (train_data, test_data) in enumerate(zip(train_stream, test_stream)):
        ...
        trained_model, _, _, _, _, new_task_first_loss = trainES(train_data, test_data, trained_model, criterion,
                                                                 optimizer, scheduler, config.max_epoch,
                                                                 device, config.patience, func_sim=True,
                                                                 task_id=task_id[j])

        # test model on basic task and task j
        with torch.no_grad():
            _, acc_array2[j, 0] = test(basic_task_test_data, trained_model, criterion, device, task_id=task_id[j])
            _, acc_array2[j, 1] = test(test_stream[j], trained_model, criterion, device, task_id=task_id[j])
